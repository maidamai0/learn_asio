...
/// TODO diffrence between task and work
struct scheduler::task_cleanup {
  ~task_cleanup() {
    // move works on the thread to scheduler
...
scheduler::scheduler(asio::execution_context &ctx, int concurrency_hint)
    : asio::detail::execution_context_service_base<scheduler>(ctx),
      ...
      task_(0), task_interrupted_(true), outstanding_work_(0), stopped_(false),
      shutdown_(false), concurrency_hint_(concurrency_hint) {
...
void scheduler::init_task() {
  mutex::scoped_lock lock(mutex_);
  if (!shutdown_ && !task_) {
    // get/create a new reacotr(epollon linux)
    task_ = &use_service<reactor>(this->context());
    op_queue_.push(&task_operation_); // first operation is **task** operation
...
std::size_t scheduler::run(asio::error_code &ec) {
  ...
  thread_call_stack::context ctx(this, this_thread); //  add to stack

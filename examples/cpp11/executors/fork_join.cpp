...
private:
  template <class Func> struct function : fork_join_pool::function_base {
...
static fork_join_pool exe_ctx;

template <class Iterator> void fork_join_sort(Iterator begin, Iterator end) {
  auto n = end - begin;
  if (n > 32768) {
    {
      fork_executor executor(exe_ctx);
      join_guard join(executor);
      dispatch(executor, [=] { fork_join_sort(begin, begin + n / 2); });
      dispatch(executor, [=] { fork_join_sort(begin + n / 2, end); });
    }
...
int main(int argc, char *argv[]) {
  ...
  std::vector<double> vec(static_cast<size_t>(std::atoll(argv[1])));
